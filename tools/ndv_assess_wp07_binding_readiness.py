...
REQUIRED_BINDING_FIELDS = {"binding_id", "binding_ref", "binding_file_sha256", "qualification_ref", "qualification_file_sha256", "exact_executor_identity", "surface_class"}
...
def verify_binding(root:Path,role:str,b:dict[str,Any])->None:
    ...
    bref=resolve(root,b.get("binding_ref"),f"{role}.binding_ref"); qref=resolve(root,b.get("qualification_ref"),f"{role}.qualification_ref")
    ...
    if surface=="SUBSCRIPTION_EXECUTOR_PINNED":
        ...
        if (mref.parent/"executor-binding.json").resolve()!=bref or (mref.parent/"qualification.json").resolve()!=qref: raise ValueError(f"{role}: evidence manifest does not bind referenced binding/qualification files")
        if sealed["binding"].get("binding_id")!=b.get("binding_id"): raise ValueError(f"{role}: sealed bundle binding_id mismatch")
        sref=resolve(root,b.get("execution_surface_ref"),f"{role}.execution_surface_ref")
        if sha_file(sref)!=b.get("execution_surface_file_sha256"): raise ValueError(f"{role}: execution surface hash mismatch")
    ...
    for tid,(name,required_roles) in EXPECTED.items():
        ...
        if tid=="B3":
            ...
            if policy:
                ...
                ready=bool(policy); missing=[] if ready else ["FAMILY_POLICY_MAP"]
            else: ready,missing=False,["FAMILY_POLICY_MAP"]
        else:
        ...
        status[tid]={"ready":ready,"missing_roles":missing,"declared_status":declared}; all_ready=all_ready and ready
    return {"schema_id":"ndv-p1-wp07-binding-readiness-v1","status":"ALL_TREATMENTS_BOUND" if all_ready else "BINDING_COVERAGE_INCOMPLETE","registry_ref":str(registry_path.resolve()),"registry_file_sha256":sha_file(registry_path.resolve()),"treatments":status,"all_treatments_ready":all_ready,"matrix_materialization_release":"YES" if all_ready else "NO","treatment_execution":"NOT_EXECUTED","holdout_access":"NONE"}
def main()->int:
    ap=argparse.ArgumentParser(description=__doc__); ap.add_argument("--registry",type=Path,default=Path("experiments/p1/wp07-treatment-bindings-v1.json")); ap.add_argument("--artifact-root",type=Path,default=Path(".")); ap.add_argument("--out",type=Path); args=ap.parse_args()
    try: result=assess(args.registry.resolve(),args.artifact_root.resolve())
    except (OSError,ValueError,json.JSONDecodeError) as exc: print(json.dumps({"status":"FAIL","reason":"WP07_BINDING_READINESS_BLOCKED","detail":str(exc)},indent=2)); return 2
    if args.out:
