...
Separate from executor invocation: consumes a pending executor-stage report,
reapplies only candidate.diff to a fresh container from the exact WP-06 audited
image, runs frozen tests with network none, parses frozen verifier IDs, and emits
YES/NO/INCONCLUSIVE.
"""
...
VERIFY_TOKEN="P1S2_VERIFY_FROZEN_DEVELOPMENT_RUN";PASS="PASSED"
...
def resolve(root:Path,value:Any,label:str)->Path:
 ...
 return p
def run(argv:list[str],timeout:float|None=None)->subprocess.CompletedProcess[str]:return subprocess.run(argv,capture_output=True,text=True,check=False,timeout=timeout)
def verify_sources(executor_report_path:Path,artifact_root:Path)->dict[str,Any]:
 ...
 source=report.get("source_chain") or {};run_spec=resolve(artifact_root,source.get("run_spec_ref"),"run_spec_ref")
 ...
 candidate=report.get("candidate") or {};diff=resolve(executor_report_path.parent,candidate.get("diff_ref"),"candidate.diff")
 ...
 task=spec.get("task") or {};admission=resolve(artifact_root,task.get("admission_record_ref"),"admission_record_ref")
 ...
 audit=ar.get("audit") or {};base_run=resolve(artifact_root,audit.get("base_run_ref"),"base_run_ref")
 ...
 focal=resolve(artifact_root,(spec.get("oracle") or {}).get("focal_verifier_ref"),"focal_verifier_ref");preservation=resolve(artifact_root,(spec.get("oracle") or {}).get("preservation_ref"),"preservation_ref");fp,pp=load(focal),load(preservation)
...
def verifier_timeout_seconds(spec:dict[str,Any],artifact_root:Path)->float:
 b=spec.get("budgets") or {};bp=resolve(artifact_root,b.get("budget_contract_ref"),"budget_contract_ref")
 if sha_file(bp)!=b.get("budget_contract_file_sha256"):raise ValueError("budget contract hash mismatch during verification")
 contract=load(bp)
 if contract.get("schema_id")!="ndv-p1-wp07-execution-budgets-v1" or contract.get("status")!="PROSPECTIVE_FROZEN_NOT_EXECUTED":raise ValueError("invalid verification budget contract")
 reserve=contract.get("verification_reserve_ms");executor=b.get("executor_timeout_ms");run_timeout=b.get("run_timeout_ms")
 if not all(isinstance(x,int) and x>0 for x in (reserve,executor,run_timeout)):raise ValueError("positive frozen verifier budgets required")
 if contract.get("executor_timeout_ms_per_hop")!=executor:raise ValueError("executor budget drift during verification")
 frozen=(contract.get("treatments") or {}).get((spec.get("treatment") or {}).get("id")) or {}
 if frozen.get("run_timeout_ms")!=run_timeout or frozen.get("retry_limit")!=b.get("retry_limit") or frozen.get("escalation_limit")!=b.get("escalation_limit"):raise ValueError("run budget drift during verification")
 remaining=run_timeout-executor
 if remaining<=0:raise ValueError("no frozen run budget remains for verification")
...
def parse_markers(stdout:str,count:int)->dict[str,Any]:
 import re
 hm=re.search(r"^__NDV_HEAD__=([0-9a-f]{40})$",stdout,re.M);cm=re.search(r"^__NDV_CLEAN__=(YES|NO)$",stdout,re.M);am=re.search(r"^__NDV_APPLY_RC__=(\d+)$",stdout,re.M);found={int(i):int(rc) for i,rc in re.findall(r"^__NDV_CMD_(\d+)_RC__=(\d+)$",stdout,re.M)}
...
def execute_candidate(image_digest:str,workdir:str,base_sha:str,commands:list[str],diff:Path,timeout_seconds:float)->dict[str,Any]:
 ...
 script=build_script(base_sha,commands);create=run(["docker","create","--network","none","-w",workdir,"--entrypoint","/bin/bash",image_digest,"-lc",script],timeout=60)
 ...
 try:
  ...
  t0=time.monotonic()
  try:proc=run(["docker","start","-a",cid],timeout=timeout_seconds);timed_out=False;stdout,stderr,rc=proc.stdout or "",proc.stderr or "",proc.returncode
 ...
 finally:run(["docker","rm","-f",cid],timeout=30)
...
def evaluate(parsed:dict[str,str],focal:dict[str,Any],preservation:dict[str,Any])->dict[str,Any]:
 normalized={normalize(str(k)):str(v) for k,v in parsed.items()};focal_tests=[normalize(str(x)) for x in focal.get("tests",[])];pres_tests=[normalize(str(x)) for x in preservation.get("tests",[])]
 ...
 fobs=[{"test":t,"observed":normalized.get(t)} for t in focal_tests];pobs=[{"test":t,"observed":normalized.get(t)} for t in pres_tests];missing=[x["test"] for x in fobs+pobs if x["observed"] is None]
...
def verify(executor_report_path:Path,artifact_root:Path,upstream_root:Path,out_dir:Path,verify_token:str)->dict[str,Any]:
 ...
 src=verify_sources(executor_report_path.resolve(),artifact_root.resolve());timeout_seconds=verifier_timeout_seconds(src["spec"],artifact_root.resolve());upstream=verify_upstream(upstream_root.resolve());parser_meta=src["focal"].get("parser") or {}
 ...
 parser=import_parser(upstream_root.resolve(),parser_meta.get("name"));base=src["base"];commands=base.get("test_commands")
 ...
 image=base.get("image_digest");workdir=base.get("workdir");started=utc_now();result=execute_candidate(image,workdir,src["spec"]["task"]["base_sha"],commands,src["diff"],timeout_seconds);completed=utc_now();markers=result["markers"]
 ...
 if result["timed_out"]:decision={"outcome":"INCONCLUSIVE","failure_attribution":"RESOURCE_LIMIT","reason":"verification run exceeded frozen verification reserve"}
 elif not harness_ok:
  ...
  parsed_raw=parser(result["stdout"]);decision={"outcome":"INCONCLUSIVE","failure_attribution":"ORACLE_DEFECT","reason":"frozen parser returned non-object"} if not isinstance(parsed_raw,dict) else evaluate(parsed_raw,src["focal"],src["preservation"])
