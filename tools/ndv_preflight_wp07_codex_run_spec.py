#!/usr/bin/env python3
"""Preflight frozen WP-07 Codex bindings without exposing a task.

The normal CLI remains single-hop only (B0/B1/B3). Cascade controllers may call
preflight_hop() with an explicit frozen role. No task prompt is sent to Codex.
"""
...
def verify_budget(spec:dict[str,Any],artifact_root:Path)->dict[str,Any]:
 tid=((spec.get("treatment") or {}).get("id"));budgets=spec.get("budgets")
 ...
 return {"path":bref,"contract":contract,"executor_timeout_ms":executor_timeout,"run_timeout_ms":run_timeout,"retry_limit":0,"escalation_limit":expected_escalation}
def _verify_binding(spec:dict[str,Any],artifact_root:Path,b:dict[str,Any],budget:dict[str,Any],role:str|None)->dict[str,Any]:
 if b.get("surface_class")!="SUBSCRIPTION_EXECUTOR_PINNED":raise ValueError(f"{role or 'selected binding'} is not a Codex subscription binding")
 ...
 model=(binding.get("model") or {}).get("identity");exe=Path((binding.get("scaffold") or {}).get("executable_path") or "").resolve()
 if not exe.is_file():raise ValueError(f"frozen Codex executable missing: {exe}")
 ver=subprocess.run([str(exe),"--version"],capture_output=True,text=True,check=False,timeout=30);raw=(ver.stdout or "")+(ver.stderr or "")
 ...
 hp=subprocess.run([str(exe),surface["mode"],"--help"],capture_output=True,text=True,check=False,timeout=30);help_text=(hp.stdout or "")+(hp.stderr or "")
 ...
 return {"schema_id":"ndv-p1-wp07-codex-run-preflight-v1","status":"CODEX_RUN_SPEC_PREFLIGHT_PASS","run_id":spec.get("run_id"),"treatment_id":((spec.get("treatment") or {}).get("id")),"hop_role":role,"binding_id":b.get("binding_id"),"candidate_id":binding.get("candidate_id"),"model":model,"execution_surface_ref":b.get("execution_surface_ref"),"execution_surface_file_sha256":b.get("execution_surface_file_sha256"),"budget_contract_ref":str(budget["path"]),"budget_contract_file_sha256":sha_file(budget["path"]),"executor_timeout_ms":budget["executor_timeout_ms"],"run_timeout_ms":budget["run_timeout_ms"],"retry_limit":budget["retry_limit"],"escalation_limit":budget["escalation_limit"],"codex_version_observed":raw.strip(),"argv_template":argv,"environment_variables_to_remove":surface["environment_variables_removed"],"task_prompt_exposed":False,"treatment_execution":"NOT_EXECUTED","holdout_access":"NONE"}
def _load_spec(spec_path:Path)->dict[str,Any]:
 ...
 return spec
def preflight_hop(spec_path:Path,artifact_root:Path,role:str)->dict[str,Any]:
 spec=_load_spec(spec_path);budget=verify_budget(spec,artifact_root);tid=((spec.get("treatment") or {}).get("id"));bindings=(spec.get("treatment") or {}).get("bindings") or {}
 if tid not in CASCADE:raise ValueError(f"{tid}: explicit per-hop preflight is only for cascade treatments")
 if role not in bindings:raise ValueError(f"{tid}: frozen hop role not found: {role}")
 b=bindings[role]
 if not isinstance(b,dict):raise ValueError(f"{tid}.{role}: binding must be object")
 return _verify_binding(spec,artifact_root,b,budget,role)
def preflight(spec_path:Path,artifact_root:Path)->dict[str,Any]:
 spec=_load_spec(spec_path);tid=((spec.get("treatment") or {}).get("id"))
 if tid in CASCADE:raise ValueError(f"{tid}: cascade treatment requires explicit per-hop preflight")
 if tid not in SINGLE_HOP:raise ValueError(f"unsupported single-hop Codex treatment: {tid!r}")
 budget=verify_budget(spec,artifact_root);bindings=(spec.get("treatment") or {}).get("bindings") or {}
 ...
 return _verify_binding(spec,artifact_root,subscription[0],budget,None)
def main()->int:
