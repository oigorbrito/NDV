...
class B3PolicyTests(unittest.TestCase):
    def mk_candidate(self,root,model,cid):
        d=root/model; d.mkdir(); q={"schema_id":"ndv-p1-wp07-codex-subscription-qualification-v1","status":"S0_READY","candidate_id":cid,"requested_model":model,"observed_models":[model],"exact_model_observed":True}; qp=d/"qualification.json"; qp.write_text(json.dumps(q)); b={"schema_id":"ndv-p1-wp07-executor-binding-v1","status":"QUALIFIED","binding_id":"B-"+model,"candidate_id":cid,"model":{"identity":model},"qualification_file_sha256":mod.sha_file(qp),"dynamic_routing":False,"implicit_fallback":False,"exact_executor_identity":"codex+"+model}; (d/"executor-binding.json").write_text(json.dumps(b)); return d
