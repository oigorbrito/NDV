...
class OracleInterpreterTests(unittest.TestCase):
    def setUp(self):
        self.row = {"instance_id": "repo__task-1", "base_commit": "a" * 40, "FAIL_TO_PASS": ["test_bug"], "PASS_TO_PASS": ["test_regression"], "install_config": {"log_parser": "fixture"}}
        self.run = {"candidate_id": "S2W01-repo__task-1", "source_instance_id": "repo__task-1", "base_revision": "a" * 40, "gold_patch_applied": False, "test_patch_applied": False, "harness_integrity": "PASS"}

    def parser(self, _log):
        return {"test_bug": "FAILED", "test_regression": "PASSED"}

    def test_expected_base_behavior(self):
        self.assertEqual(mod.interpret(self.row, self.run, "ignored", self.parser)["classification"], "EXPECTED_BASE_BEHAVIOR")
    ...
    def test_empty_parser_result_is_environment_inconclusive(self):
        self.assertEqual(mod.interpret(self.row, self.run, "ignored", lambda _: {})["classification"], "ENVIRONMENT_INCONCLUSIVE")

    def test_patch_application_is_rejected(self):
        run = dict(self.run); run["gold_patch_applied"] = True
        with self.assertRaises(RuntimeError):
            mod.interpret(self.row, run, "ignored", self.parser)

    def test_harness_integrity_is_required(self):
        run = dict(self.run); run["harness_integrity"] = "FAIL"
        with self.assertRaises(RuntimeError):
            mod.interpret(self.row, run, "ignored", self.parser)

    def test_identity_mismatch_is_rejected(self):
        run = dict(self.run); run["base_revision"] = "b" * 40
        with self.assertRaises(RuntimeError):
            mod.interpret(self.row, run, "ignored", self.parser)
