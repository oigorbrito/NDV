...
class PromotionTests(unittest.TestCase):
    ...
    def make_candidate(self, root:Path, model:str, cid:str, roles:list[str])->Path:
        ...
        q={"schema_id":"ndv-p1-wp07-codex-subscription-qualification-v1","status":"S0_READY","candidate_id":cid,"requested_model":model,"observed_models":[model],"exact_model_observed":True,"candidate_roles":roles}; qp=d/"qualification.json"; qp.write_text(json.dumps(q),encoding="utf-8")
        b={"schema_id":"ndv-p1-wp07-executor-binding-v1","status":"QUALIFIED","binding_id":"B-"+model,"candidate_id":cid,"surface_class":"SUBSCRIPTION_EXECUTOR_PINNED","exact_executor_identity":"codex+"+model,"model":{"identity":model},"auth_path":"CHATGPT_SUBSCRIPTION","api_key_routing_forbidden":True,"dynamic_routing":False,"implicit_fallback":False,"retry_limit":0,"escalation_limit":0,"qualification_file_sha256":mod.sha_file(qp),"candidate_roles":roles}; (d/"executor-binding.json").write_text(json.dumps(b),encoding="utf-8")
    ...
    def test_tampered_qualification_bundle_blocks(self):
        with tempfile.TemporaryDirectory() as tmp:
            r=Path(tmp); reg=self.make_registry(r); sol=self.make_candidate(r,"gpt-5.6-sol","CODEX-PLUS-GPT-5.6-SOL",["PRIMARY_STRONG","ESCALATION_STRONG"]); luna=self.make_candidate(r,"gpt-5.6-luna","CODEX-PLUS-GPT-5.6-LUNA",["PRIMARY_ECONOMIC"])
            (luna/"evidence"/"candidate.diff").write_text("tampered",encoding="utf-8")
