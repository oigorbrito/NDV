...
def verify_dir(root:Path,expected_model:str,expected_candidate:str,required_roles:set[str],artifact_root:Path)->dict[str,Any]:
    ...
    return {"q":q,"b":b,"qpath":qpath,"bpath":bpath,"manifest_path":sealed["manifest_path"],"surface_path":qsp}
def record(v:dict[str,Any],artifact_root:Path)->dict[str,Any]:
    b=v["b"]
    return {"binding_id":b["binding_id"],"binding_ref":ref(v["bpath"],artifact_root),"binding_file_sha256":sha_file(v["bpath"]),"qualification_ref":ref(v["qpath"],artifact_root),"qualification_file_sha256":sha_file(v["qpath"]),"evidence_manifest_ref":ref(v["manifest_path"],artifact_root),"evidence_manifest_sha256":sha_file(v["manifest_path"]),"execution_surface_ref":ref(v["surface_path"],artifact_root),"execution_surface_file_sha256":sha_file(v["surface_path"]),"exact_executor_identity":b["exact_executor_identity"],"surface_class":"SUBSCRIPTION_EXECUTOR_PINNED"}
