...
Requires a valid WP-07 development release, a fully bound treatment registry, and
ADMITTED_FROZEN task records. The output is a plan/spec inventory only.
"""
...
def sha(value: Any) -> str: return hashlib.sha256(canonical(value)).hexdigest()
...
def materialize(release_path:Path, protocol_path:Path, registry_path:Path, admission_paths:list[Path], artifact_root:Path, shaping_mode:str)->dict[str,Any]:
    if shaping_mode not in SHAPING_MODES: raise ValueError("invalid shaping mode")
    release=verify_release(release_path); protocol=verify_protocol(protocol_path); reg=verify_registry(registry_path,artifact_root)
    records=[verify_admission(p) for p in admission_paths]
    ...
    shapings=["S0_RAW_TASK"] if shaping_mode=="S0_ONLY" else ["S0_RAW_TASK","S1_DETERMINISTIC_TASK_SHAPING"]
    specs=[]
        ...
        for tid in sorted(TREATMENTS):
            ...
            for shaping in shapings:
                specs.append({
                    ...
                    "oracle":{"focal_verifier_ref":audit.get("focal_verifier_ref"),"focal_verifier_sha256":audit.get("focal_verifier_sha256"),"preservation_ref":audit.get("preservation_ref"),"preservation_sha256":audit.get("preservation_sha256"),"environment_ref":audit.get("environment_ref"),"environment_sha256":audit.get("environment_sha256")},
                    "accounting":{"primary":protocol["primary_metric"],"failure_cost_retained":True,"inconclusive_cost_retained":True,"missing_telemetry_is_zero":False},
    ...
    return {"schema_id":"ndv-p1-wp07-run-matrix-v1","status":"RUN_SPECS_MATERIALIZED_NOT_EXECUTED","shaping_mode":shaping_mode,"task_count":len(records),"treatment_count":5,"run_spec_count":len(specs),"release_ref":str(release_path.resolve()),"release_file_sha256":sha_file(release_path.resolve()),"protocol_ref":str(protocol_path.resolve()),"protocol_file_sha256":sha_file(protocol_path.resolve()),"binding_registry_ref":str(registry_path.resolve()),"binding_registry_file_sha256":sha_file(registry_path.resolve()),"run_specs":specs,"treatment_execution":"NOT_EXECUTED","holdout_access":"NONE"}
def main()->int:
    ap=argparse.ArgumentParser(description=__doc__); ap.add_argument("--release",required=True,type=Path); ap.add_argument("--protocol",type=Path,default=Path("experiments/p1/wp07-development-comparison-protocol-v1.json")); ap.add_argument("--bindings",type=Path,default=Path("experiments/p1/wp07-treatment-bindings-v1.json")); ap.add_argument("--admission-record",action="append",default=[],type=Path); ap.add_argument("--artifact-root",type=Path,default=Path(".")); ap.add_argument("--shaping-mode",choices=sorted(SHAPING_MODES),default="S0_ONLY"); ap.add_argument("--out",required=True,type=Path); args=ap.parse_args()
    try: result=materialize(args.release.resolve(),args.protocol.resolve(),args.bindings.resolve(),args.admission_record,args.artifact_root.resolve(),args.shaping_mode)
    ...
    args.out.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n",encoding="utf-8"); print(json.dumps({"status":result["status"],"task_count":result["task_count"],"run_spec_count":result["run_spec_count"],"shaping_mode":result["shaping_mode"]},indent=2)); return 0
