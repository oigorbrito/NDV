...
TREATMENTS={"B0","B1","B2","B3","B4"}; SHAPING_MODES={"S0_ONLY","PAIRED_S0_S1"}
...
def materialize(release_path:Path,protocol_path:Path,registry_path:Path,admission_paths:list[Path],artifact_root:Path,shaping_mode:str)->dict[str,Any]:
 ...
 verify_release(release_path);protocol=verify_protocol(protocol_path);reg=verify_registry(registry_path,artifact_root);records=[verify_admission(p) for p in admission_paths]
 ...
 for r,p in zip(records,admission_paths):
  ...
  for tid in sorted(TREATMENTS):
   ...
   for shaping in shapings:
    specs.append({"schema_id":"ndv-p1-s2-run-spec-v1","run_id":run_id(r["candidate_id"],tid,shaping),"phase":"P1-S2","task":{"task_id":r["candidate_id"],"family":family,"repository":r["repository"],"base_sha":r["base_revision"],"task_statement_sha256":source.get("task_statement_sha256"),"admission_record_ref":str(p.resolve()),"admission_record_file_sha256":sha_file(p.resolve()),"admission_record_sha256":r["record_sha256"],"solution_isolation":"SOLUTION_NOT_AVAILABLE_TO_EXECUTOR"},"treatment":{"id":tid,"bindings":binding,"shaping":shaping,"rollout":1,"repetition":1},"oracle":{"focal_verifier_ref":audit.get("focal_verifier_ref"),"focal_verifier_sha256":audit.get("focal_verifier_sha256"),"preservation_ref":audit.get("preservation_ref"),"preservation_sha256":audit.get("preservation_sha256"),"environment_ref":audit.get("environment_ref"),"environment_sha256":audit.get("environment_sha256")},"accounting":{"primary":protocol["primary_metric"],"failure_cost_retained":True,"inconclusive_cost_retained":True,"missing_telemetry_is_zero":False},"holdout":"DEVELOPMENT_ONLY","execution_status":"NOT_EXECUTED"})
