...
class MaterializationTests(unittest.TestCase):
    def fixture(self, root: Path, data: bytes = b"parquet-bytes"):
        ...
        environment.write_text(json.dumps({
            ...
            "network_during_extraction": "FORBIDDEN", "model_execution": "NONE",
            "treatment_execution": "NOT_EXECUTED", "holdout_access": "NONE",
        }), encoding="utf-8")
        return parquet, receipt, snapshot, contract, environment

    def test_verified_acquisition_is_accepted(self):
        with tempfile.TemporaryDirectory() as tmp:
            parquet, receipt, snapshot, contract, _ = self.fixture(Path(tmp))
            result = mod.verify_acquisition(parquet, receipt, snapshot, contract)
    ...
    def test_environment_is_byte_bound(self):
        with tempfile.TemporaryDirectory() as tmp:
            *_, environment = self.fixture(Path(tmp))
            result = mod.verify_environment(environment)
            self.assertEqual(result["required_pyarrow_version"], "25.0.1")
            self.assertEqual(result["environment_file_sha256"], mod.sha256_file(environment))
    ...
    @mock.patch.object(mod, "verify_environment")
    @mock.patch.object(mod, "verify_acquisition")
    def test_main_writes_materialization_receipt_after_full_quarantine(self, verify, verify_env, run_extraction):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            environment = root / "environment.json"; environment.write_text("{}")
            ...
            verify_env.return_value = {"environment_ref": str(environment), "environment_file_sha256": "e" * 64, "python_version": sys.version.split()[0], "required_pyarrow_version": "25.0.1"}
            extraction_result = {"status": "PASS", "schema_id": "ndv-p1-s2-pinned-row-extraction-v3", "environment_file_sha256": "e" * 64, "python_version": sys.version.split()[0], "pyarrow_version": "25.0.1"}
            class P:
                returncode = 0
            ...
            argv = ["prog", "--parquet", str(parquet), "--acquisition-receipt", str(acquisition), "--snapshot", str(snapshot), "--contract", str(contract), "--environment", str(environment), "--wave", str(wave), "--extractor", str(extractor), "--out-root", str(out_root)]
            ...
            self.assertEqual(receipt_payload["schema_id"], "ndv-p1-s2-wave-materialization-receipt-v2")
            self.assertEqual(receipt_payload["status"], "MATERIALIZED_QUARANTINED")
            self.assertEqual(receipt_payload["extraction_pyarrow_version"], "25.0.1")
