...
def source_origin(source_repo: Path) -> str:
    ...
    if proc.returncode != 0 or not proc.stdout.strip():
        raise RuntimeError("source repo must expose an origin remote for historical materialization")
...
def materialize_exact_base(source_repo: Path, workspace: Path, base_sha: str, evidence_dir: Path) -> str:
    ...
    if clone.returncode != 0:
        raise RuntimeError("historical workspace clone from origin failed before task exposure")

    fetch = run(["git", "fetch", "--no-tags", "origin", base_sha], workspace, 600)
    write_log(evidence_dir / "materialization-fetch.log", fetch)
    if fetch.returncode != 0:
        raise RuntimeError(f"exact historical base fetch failed before task exposure: {fetch.stderr or fetch.stdout}")

    object_check = run(["git", "cat-file", "-e", f"{base_sha}^{{tree}}"], workspace, 30)
    write_log(evidence_dir / "materialization-object-check.log", object_check)
    if object_check.returncode != 0:
        raise RuntimeError("fetched historical commit tree is unavailable before task exposure")

    checkout = run(["git", "checkout", "--detach", base_sha], workspace, 120)
    write_log(evidence_dir / "materialization-checkout.log", checkout)
    if checkout.returncode != 0:
        raise RuntimeError(f"historical base checkout failed before task exposure: {checkout.stderr or checkout.stdout}")
...
def main() -> int:
    ...
    ap.add_argument("--source-repo", required=True, type=Path, help="Local oigorbrito/metaO clone used to resolve the canonical authenticated origin")
    ...
    if base_focal.returncode == 0:
        raise SystemExit("ORACLE_DEFECT_PRE_EXPOSURE: structural focal unexpectedly passes on untouched base")
