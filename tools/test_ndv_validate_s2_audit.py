...
BASE = {
    ...
    "audits": [{
        ...
        "verifier_independent": None, "environment_ref": None, "environment_sha256": None, "treatment_execution": "NOT_EXECUTED",
    }],
}
...
def complete_pass(rec):
    rec.update({
        ...
        "verifier_independent": True, "environment_ref": "environment/image.json", "environment_sha256": "1" * 64,
        "gold_patch_applied_in_base_mode": False, "test_patch_applied_in_base_mode": False,
    })
...
class AuditValidatorTests(unittest.TestCase):
    ...
    def test_audit_pass_requires_complete_evidence(self):
        ...
        self.assertTrue(any("image_digest" in e for e in errors)); self.assertTrue(any("harness_integrity" in e for e in errors)); self.assertTrue(any("environment_sha256" in e for e in errors))
    ...
    def test_environment_hash_required(self):
        payload = copy.deepcopy(BASE); rec = payload["audits"][0]; complete_pass(rec); rec["environment_sha256"] = None
        self.assertTrue(any("environment_sha256" in e for e in validate(payload)))
