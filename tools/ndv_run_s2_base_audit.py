...
SCHEMA = "ndv-p1-s2-base-audit-run-v3"
...
def validate_pinned_digest(image_ref: str, image_digest: str) -> str:
    ...
    if proc.returncode != 0 or not proc.stdout.strip():
        raise RuntimeError(f"pinned image digest unavailable locally: {image_digest}")
    return image_digest
...
def validate_row(row: dict[str, Any], candidate: dict[str, Any]) -> tuple[list[str], str, str, list[str], str | None]:
    ...
    try:
        ...
        errors.append(str(exc)); commands = []
    ...
    if not isinstance(image_ref, str) or not image_ref:
        errors.append("candidate image_ref missing"); image_ref = ""
...
def execute_base(image_digest: str, workdir: str, base_revision: str, commands: list[str], timeout: float) -> dict[str, Any]:
    ...
    try:
        proc = run(argv, timeout=timeout); timed_out, returncode, stdout, stderr = False, proc.returncode, proc.stdout, proc.stderr
    except subprocess.TimeoutExpired as exc:
        timed_out, returncode = True, 124; stdout, stderr = exc.stdout or "", (exc.stderr or "") + f"\ntimeout after {timeout} seconds\n"
...
def main() -> int:
    ...
    wave = json.loads(args.wave.read_text(encoding="utf-8")); artifact = json.loads(args.admission_row.read_text(encoding="utf-8"))
    ...
    try:
        image_digest = validate_pinned_digest(image_ref, args.image_digest)
    except (OSError, RuntimeError, subprocess.TimeoutExpired) as exc:
        report = {"schema_id": SCHEMA, "candidate_id": args.candidate_id, "status": "ENVIRONMENT_BLOCKED", "reason": str(exc), "image_ref": image_ref, "requested_image_digest": args.image_digest, "gold_patch_applied": False, "test_patch_applied": False, "observed_at": utc_now(), "treatment_execution": "NOT_EXECUTED", "holdout_access": "NONE"}
        write_json(args.out / "base-audit-run.json", report); print(json.dumps(report, indent=2, sort_keys=True)); return 3
    ...
    stdout_path.write_text(result.pop("stdout"), encoding="utf-8"); stderr_path.write_text(result.pop("stderr"), encoding="utf-8")
    markers = result["markers"]
    harness_integrity = markers["observed_head"] == candidate["base_revision"] and markers["clean_before_tests"] is True and markers["all_command_markers_present"]
    report = {
        "schema_id": SCHEMA, "candidate_id": args.candidate_id, "source_instance_id": candidate["source_instance_id"], "repository": candidate["repository"], "base_revision": candidate["base_revision"],
        "admission_row_ref": str(args.admission_row), "admission_row_file_sha256": sha256_file(args.admission_row), "admission_full_row_sha256": sha256_bytes(canonical_bytes(row)),
        "image_ref": image_ref, "image_digest": image_digest, "runtime": "docker", "network": "none", "workdir": workdir, "test_commands": commands, "test_commands_sha256": sha256_bytes(canonical_bytes(commands)), "log_parser_name": parser_name,
        "gold_patch_applied": False, "test_patch_applied": False, "process": result, "harness_integrity": "PASS" if harness_integrity else "FAIL",
        ...
        "observed_at": utc_now(), "classification": "BASE_RUN_RECORDED" if harness_integrity else "HARNESS_INVALID", "note": "Test failures are interpreted separately; missing HEAD/clean/command markers invalidate the harness run.", "treatment_execution": "NOT_EXECUTED", "holdout_access": "NONE",
    }
    report_path = args.out / "base-audit-run.json"; write_json(report_path, report)
